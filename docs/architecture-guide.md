# OpenSpec + Superpowers + GSD 三合一架構說明書
...
### OpenSpec — 規格驅動開發（Spec-Driven Development）

OpenSpec 是一套以「規格文件活在 repo 裡」為核心理念的輕量開發框架。它的設計哲學是：

- **Spec 是真相來源（Single Source of Truth）：** 所有系統行為的定義都寫在 `openspec/specs/` 下的 Markdown 檔案中，而非散落在 Jira ticket、Slack 訊息或團隊記憶裡。
- **Change 是最小變更單位：** 每次功能修改都是一個獨立的 change（放在 `openspec/changes/` 下），包含 proposal（為什麼做）、design（怎麼做）、tasks（做哪些事）三份文件。
...
**OpenSpec 解決的問題：** 在 AI Coding 中，Agent 缺乏對「系統應該長什麼樣」的結構化理解。沒有 spec，Agent 只能依賴 chat context 來猜測需求，結果容易偏離。
...
| # | 問題 | 症狀 | 本架構如何解決 |
|---|------|------|----------------|
| 1 | **Context 遺失** | Agent 不記得上一輪對話講過什麼，反覆問同樣的問題 | OpenSpec 的 spec 和 change 文件**活在 repo 裡**，Agent 每次開始工作都先讀檔案，不依賴 chat 記憶 |
| 2 | **Scope 漂移** | 要求做 A 功能，Agent 自作主張加了 B、C、D | OpenSpec 的 proposal + tasks 嚴格定義「只做什麼」，AGENTS.md 規定「flag scope drift before coding」 |
...
「做什麼」         「怎麼做得好」         「按什麼順序做」
 ...
 │                                                   │
 │  openspec/specs/       ← 規格真相（OpenSpec）       │
 │  openspec/changes/     ← 變更包（OpenSpec）         │
...
**核心價值：** 單獨使用任何一個框架都只解決部分問題。OpenSpec 沒有執行順序；Superpowers 沒有規格管理；GSD 沒有 spec delta 機制。三者整合後，從「需求定義 → 計畫 → 執行 → 驗證 → 規格回寫」的完整循環才真正閉合。
...
| Layer 2 | OpenSpec | 定義系統的「應然」狀態，每個 capability 一份 spec | `openspec/specs/<cap>/spec.md` |
| Layer 3 | OpenSpec | 每次變更的完整記錄：為什麼做、怎麼做、做了什麼 | `openspec/changes/<id>/` |
| Layer 4 | OpenSpec CLI | （可選）自動化生成 change skeleton、驗證 spec 一致性 | CLI 工具，不存 repo |
...
### 4.1 `openspec/specs/<capability>/spec.md` — OpenSpec 原理

**來自 OpenSpec 的概念：Living Spec**
...
### 4.2 `openspec/changes/<change-id>/` — OpenSpec 原理

**來自 OpenSpec 的概念：Change Package**
...
- 願景與使用者問題（對應 OpenSpec 的 proposal 動機）
- 技術棧選擇
- 全域限制條件（效能、安全、相容性）
- 團隊組成

**為什麼需要它？** OpenSpec 的 spec 是分散在各 capability 下的，缺少一個「全專案鳥瞰圖」。PROJECT.md 補上這個缺口。
...
```
Milestone 1: MVP
  └── Phase 1: add-auth       ← 對應 openspec/changes/add-auth/
  └── Phase 2: add-billing    ← 對應 openspec/changes/add-billing/
  └── Phase 3: add-dashboard  ← 對應 openspec/changes/add-dashboard/
```

**關鍵映射：1 個 GSD Phase = 1 個 OpenSpec Change**

這是整合架構最核心的設計決策。GSD 負責「按什麼順序做」，OpenSpec 負責「做的內容是什麼」。Phase 編號和 change-id 一一對應，不會產生兩套追蹤系統。
...
| 規則 | 來源框架 | 原理 |
|------|----------|------|
| 「先讀 spec 和 change 文件再動手」 | OpenSpec | Spec-driven：Agent 必須理解規格再行動 |
...
| 「Flag scope drift before coding」 | OpenSpec + GSD | 防止 Agent 超出 proposal 和 phase 範圍 |
...
- GSD command 和 OpenSpec change 的對應關係
- 檔案位置速查表
...
步驟 2: 理解需求（OpenSpec）
├── 讀 openspec/changes/<change-id>/proposal.md
├── 讀相關的 openspec/specs/<cap>/spec.md
└── 框架原理：OpenSpec 的 spec-driven 理念
...
步驟 4: 撰寫計畫（Superpowers + OpenSpec）
├── 觸發 writing-plans skill
├── 輸出到 openspec/changes/<change-id>/design.md
├── 輸出到 openspec/changes/<change-id>/tasks.md
└── 框架原理：Superpowers 驅動，OpenSpec 承載
...
步驟 7: Spec 回寫（OpenSpec）
├── 更新 openspec/changes/<change-id>/specs/<cap>/spec.md（delta）
├── 將 delta 合併回 openspec/specs/<cap>/spec.md（living spec）
└── 框架原理：OpenSpec 的 spec delta merge
...
│
├── openspec/                                    [OpenSpec]
│   ├── README.md                                [OpenSpec]  目錄說明
│   ├── specs/
│   │   ├── <capability-1>/spec.md               [OpenSpec]  Living spec
│   │   └── <capability-2>/spec.md               [OpenSpec]  Living spec
│   └── changes/
│       └── <first-change>/
│           ├── proposal.md                      [OpenSpec]  變更提案
│           ├── design.md                        [OpenSpec]  技術設計
│           ├── tasks.md                         [OpenSpec]  任務清單
│           └── specs/
│               └── <capability>/spec.md         [OpenSpec]  Spec delta
│
└── .planning/                                   [GSD]
...
| 框架 | 負責的檔案 | 核心職責 |
|------|-----------|----------|
| **OpenSpec** | `openspec/` 整個目錄 | 定義「系統應該是什麼樣」 |
...
| 術語 | 所屬框架 | 定義 |
|------|----------|------|
| Living Spec | OpenSpec | 隨專案演進持續更新的系統行為規格文件 |
| Change | OpenSpec | 一次功能變更的完整包（proposal + design + tasks + spec delta） |
| Spec Delta | OpenSpec | 一次 change 對 living spec 造成的行為差異 |
| Capability | OpenSpec | 系統的功能領域（如 auth、billing） |
| Skill | Superpowers | 一個可被 Agent 自動觸發的工程實踐（如 TDD、code-review） |
| Definition of Done | Superpowers | 判斷一個任務是否真正完成的檢查清單 |
| Phase | GSD | 一個可交付的功能切片，對應一個 OpenSpec change |
