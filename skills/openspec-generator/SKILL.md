...
description: OpenSpec Toolkit 原生架構師 — Agent 根據五層結構規範，動態生成與維護 spec-driven repo，支援 Greenfield/Brownfield，跨 AI 工具通用
version: 2.1.0
triggers:
  - 建立新專案
  - openspec
...
# OpenSpec-Architect-Native (純認知導航版)

## 1. 核心定位

- **角色**：OpenSpec 原生架構師
- **哲學**：**Repo 即地圖，檔案即真相**。不依賴外部 Python 腳本，由 Agent 根據專案現況動態生成符合 OpenSpec Toolkit 規範的五層結構
- **跨工具通用**：完全相容 Cursor、Claude Code、GitHub Copilot、Kiro 等所有具備檔案讀寫能力的 AI 工具

## 2. 空間感知：地圖導航機制

為解決跨 LLM 時的認知分歧，Agent 必須維護 `.openspec-map.md` 作為空間索引。

### 啟動校準流程

1. 進入專案第一件事：檢查是否存在 `.openspec-map.md`
2. 若無地圖：執行 `ls -R` 並產出地圖（目錄樹 + Capability 映射備註）
3. 每次對話開始或接手新任務：**必須先讀取地圖**確認開發上下文

### 地圖檔案結構 (.openspec-map.md)
...
Agent 必須在動手寫碼前，自主判定是否建立 `/openspec/changes/<id>/`：
...
2. **Layer 2**：建立初始 `openspec/specs/<capability>/spec.md`
...
1. **受影響範圍原則**：僅針對本次變更涉及的功能，在 `openspec/specs/` 補齊基準線規格
...
### 核心映射：1 Phase = 1 OpenSpec Change

| GSD 概念 | OpenSpec 對應 | 說明 |
|----------|--------------|------|
| Milestone | 一組相關 Changes | 產品版本或功能群組 |
| Phase | `openspec/changes/<id>/` | 單次變更執行單元 |
...
1. 讀取 `openspec/changes/<id>/spec_delta.md` 中的 GIVEN/WHEN/THEN
2. 將新行為追加到對應的 `openspec/specs/<capability>/spec.md`
...
2. **地圖同步**：每當建立新 Change 或新的 Capability 資料夾，必須立即更新 `.openspec-map.md`
3. **小步提交**：保持 Diff 最小化，優先確保測試通過
4. **跨工具共識**：Agent 應主動告知使用者：「我已根據 `.openspec-map.md` 定位上下文，現在準備執行 `tasks.md` 中的下一個任務。」
...
openspec-toolkit/
├── create_repo.py
├── .openspec-generator.yml.example
